...
tags: docker namespace cgroups unionfs aufs Server
toc: true
permalink: /docker
desc: 提到虚拟化技术，我们首先想到的一定是 Docker，经过四年的快速发展 Docker 已经成为了很多公司的生产环境中大规模使用，也不再是一个只能在开发阶段使用的玩具了。文章中会介绍 Docker 中的三大核心技术：Linux 命名空间（namespaces）、Control Groups 控制组（CGroups）和 UnionFS。
---
